...
- [ ] **Decide “rewrite vs evolve-in-place”**
  - Done when: we have an ADR that chooses either (A) a new `guyos_core_v2` crate/module path or (B) an incremental refactor plan with clear milestones and “stop-the-world” boundaries.
  - Recommendation from code review: **prefer clean re-implementation** to avoid entrenching spike debt.

- [ ] **Write ADR: hexagonal boundaries + ownership model**
...
- [ ] **Write ADR: wire protocol + compatibility guarantees**
